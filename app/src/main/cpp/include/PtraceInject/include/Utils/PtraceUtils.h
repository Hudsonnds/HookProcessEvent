...
int ptrace_getregs(pid_t pid, struct pt_regs *regs){
    ...
    if (ptrace(PTRACE_GETREGSET, pid, regset, &ioVec) < 0){
        printf("[-] ptrace_getregs: Can not get register values, io %lx, %d\n", (uintptr_t)ioVec.iov_base,(int)ioVec.iov_len);
        return -1;
    }
...
void *get_android_dlopen_ext_address(pid_t pid) {
    void *dlopen_addr;
    char sdk_ver[32];
    memset(sdk_ver, 0, sizeof(sdk_ver));
    __system_property_get("ro.build.version.sdk", sdk_ver);
    ...
    printf("[+] linker_path value:%s\n",process_libs.linker_path);
    if (atoi(sdk_ver) <= 23) { // 安卓7
        dlopen_addr = get_remote_func_addr(pid, process_libs.linker_path, (void *) android_dlopen_ext);
    } else {
        dlopen_addr = get_remote_func_addr(pid, process_libs.libdl_path, (void *) android_dlopen_ext);
    }
    printf("[+] dlopen RemoteFuncAddr:0x%lx\n", (uintptr_t) dlopen_addr);
    LOGE("[+] dlopen RemoteFuncAddr:0x%lx\n", (uintptr_t) dlopen_addr);
...
void *get_dlerror_address(pid_t pid) {
    ...
    printf("[+] dlerror Remot0eFuncAddr:0x%lx\n", (uintptr_t) dlerror_addr);
    return dlerror_addr;
}
